...
export class PlanetaryExplorerMapIntegration {
    ...
    extractDataFromResponse(response) {
        try {
            ...
            const collections = collectionMatch ? collectionMatch.map((m) => m.split(/[:\s]+/).pop()).filter(Boolean) : [];
    ...
    addBboxToMap(bbox) {
        if (!this.mapInstance || !bbox)
            return;
        try {
            // Add bbox geometry to map as a layer
            if (this.mapInstance.getLayer && this.mapInstance.getLayer('planetary-explorer-bbox')) {
                this.mapInstance.removeLayer('planetary-explorer-bbox');
            }
            if (this.mapInstance.getSource && this.mapInstance.getSource('planetary-explorer-bbox')) {
            ...
            if (this.mapInstance.sources && this.mapInstance.layers) {
                ...
                dataSource.add(bbox);
                const layer = new window.atlas.layer.PolygonLayer(dataSource, undefined, {
                    strokeColor: '#ff6b6b',
                    strokeWidth: 3,
                    strokeOpacity: 0.8,
