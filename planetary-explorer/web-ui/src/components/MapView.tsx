...
const MapView: React.FC<MapViewProps> = ({ 
  ...
  // Universal/free pin-drop mode: lets the user drop a pin without entering any
  // GEOINT module. The pin is bubbled up via onPinChange and the chat sends it
  // on the standard /api/query path so the analysis router (raster_sampling /
  // vision / terrain / etc.) can pick the right analyzer for the question.
  ...
  // Map basemap style. We default to 'satellite_road_labels' to match the
  // initial map creation below; the labels-toggle button on the left
  // toolbar swaps between this (labels visible) and 'satellite' (pure
      ...
      try {
        ...
        newMap.events.add('error', (error: any) => {
          ...
          const suppressedPatterns = [
            ...
            'symbol layout',
            'WebGL',
            // TiTiler returns empty / 204 bodies for tile coords outside an
            // item polygon (NAIP county-scale items at z<14 generate dozens
            // per pan). Azure Maps tries to decode the empty body as PNG,
            // fails, and fires this error per tile. Benign -- the layer's
            // own `bounds` already constrains the rendered footprint.
        ...
        // Add error handling -- second handler kept for surfacing genuine
        // map failures (auth, style load, etc.) to the UI. Filters out the
        // same benign-tile-decode noise the first handler suppresses so we
        // do not double-log + do not flash setMapError for transient tile
        // 404s outside an item polygon.
        newMap.events.add('error', (error: any) => {
          const errorMsg = error?.message || error?.error?.message || String(error);
          ...
          if (benignPatterns.some(p => errorMsg.includes(p))) {
            return;
          }
        ...
        newMap.events.add('error', (error: any) => {
          ...
          const suppressedPatterns = [
            ...
            'symbol layout',
            'WebGL',
            // Same NAIP / HLS empty-tile noise -- see top error handler.
            'Could not load image',
            'source image could not be decoded',
          ];
          if (suppressedPatterns.some(p => errorMsg.includes(p))) {
  ...
  useEffect(() => {
    ...
    const handleMapClick = (e: any) => {
      ...
      // Allow pin mode if either pinMode is true OR a module is selected OR free pin mode is on
      if (!pinMode && !selectedModule && !freePinMode) return;
  ...
  }, [map, mapLoaded, mapProvider, pinMode, selectedModule, freePinMode, pinState.marker, terrainAnalysisMode]);
    ...
    try {
      if (mapProvider === 'azure') {
        ...
        if (satelliteData.all_tile_urls && satelliteData.all_tile_urls.length > 1 && window.atlas) {
          ...
          // Remove any tile layers tracked from a previous render so old
          // STAC results don't stack on top of the new ones.
          if (activeTileLayersRef.current && activeTileLayersRef.current.length > 0) {
            console.log(`??? MapView: Removing ${activeTileLayersRef.current.length} tracked layers from previous render`);
            activeTileLayersRef.current.forEach((prevLayer: any) => {
              try { map.layers.remove(prevLayer); } catch (_) { /* already removed */ }
            });
          }
          ...
          allLayers.forEach((layer: any) => {
            if (layer.getId && (
              layer.getId().includes('planetary-explorer-tiles') ||
              layer.getId().includes('satellite-tiles') ||
              layer.getId().includes('planetary-computer')
            )) {
        ...
        // Remove any tile layers tracked from a previous render
        if (activeTileLayersRef.current && activeTileLayersRef.current.length > 0) {
          console.log(`??? MapView: Removing ${activeTileLayersRef.current.length} tracked layers from previous render (single-tile path)`);
          activeTileLayersRef.current.forEach((prevLayer: any) => {
            try { map.layers.remove(prevLayer); } catch (_) { /* already removed */ }
          });
        }
        ...
        if (satelliteData.tile_url && window.atlas) {
          ...
          const addTileLayer = async () => {
            try {
              ...
              // PERF: default to 256x256 tiles (no tile_scale=2 / no @2x). 512x512
              // tiles are ~4x the bytes per tile and ~4x the TiTiler render cost,
              // dominating first-paint latency. Azure Maps upscales 256 tiles for
              // display; users only notice the difference at z>=12.
              const enhancedTileUrl = tileUrl;
              ...
              if (isTileTemplate) {
                ...
                // PERF: 256x256 default (see comment above). Skip tile_scale=2.
                const highResUrl = tileUrl;

                tileLayerConfig = {
                  tileUrl: highResUrl, // 256x256 tiles for fast first-paint
                ...
                // PERF: 256x256 default. No tile_scale=2 -> tile template has no @2x suffix.
                const tilejsonUrl = tileUrl;
                ...
                // For MPC Pro mosaic tilejson, the upstream GeoCatalog enforces a
                // per-collection minimum zoom (e.g. sentinel2-fire rejects z<13
                // with HTTP 400). The TileJSON we receive is authoritative -- do
                // NOT take the min with renderingConfig.minZoom or we will spam
                // 400s for every low-zoom tile in the viewport.
                const isProMosaicTileJson = (tilejsonUrl || '').includes('/api/pro/mosaic/tilejson');
                const effectiveMinSourceZoom = isProMosaicTileJson
                  ? (tilejsonResult.tilejson?.minzoom ?? renderingConfig.minZoom ?? 0)
                  : Math.min(renderingConfig.minZoom, tilejsonResult.tilejson?.minzoom || 99);
                ...
                tileLayerConfig = {
                  ...
                  tileSize: 256, // PERF: 256 (was 512) for ~4x faster first-paint
                  bounds: fallbackBounds,
                  minSourceZoom: effectiveMinSourceZoom,
              ...
              try {
                ...
                const layerId = `planetary-explorer-tiles-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
              ...
              const errorHandler = (e: any) => {
                ...
                const suppressedPatterns = [
                  ...
                  'symbol layout',
                  'WebGL',
                  // See top-level error handler -- TiTiler empty-body tile
                  // responses outside the NAIP/HLS item polygon spam this
                  // event at low zoom. Suppress to keep DevTools usable.
                  'Could not load image',
                  'source image could not be decoded',
                ];
                if (suppressedPatterns.some(p => errorMsg.includes(p))) {
  ...
  const handleModuleSelect = (module: string) => {
    ...
    // Resilience module: region-scoped (no pin). The user types their question
    // (or clicks a Get Started example) and Chat.tsx routes to
    // /api/resilience/assess, formats the dossier, and dispatches map markers.
    // No welcome message — matches the no-prompt UX the user prefers for this
    // module; the Get Started gallery already documents example queries.
    if (module === 'resilience') {
      console.log('MapView: Resilience module selected - chat-driven flow');
      setPinMode(false);
    ...
    if (module === 'vision') {
      ...
      message = '**Extreme Weather selected.**';
    } else if (module === 'site_audit') {
      message = '**Site Intel selected.**';
    } else if (module === 'forecast') {
      message = '**Forecast selected.**';
    } else if (module === 'timeseries') {
      message = '**Time Series selected.**';
    }
  ...
  // Window-event bridge so other components (notably GetStartedButton) can
  // activate a module without prop drilling. The Resilience Get Started
  // "Go" button uses this to flip selectedModule to 'resilience' BEFORE
  // dispatching the chat query — otherwise Chat.tsx sees no active module
  // and falls through to the generic /api/query path instead of the
  // resilience planner endpoint.
  useEffect(() => {
    const handleSelectModuleEvent = (event: CustomEvent<{ module: string }>) => {
      const m = event?.detail?.module;
      if (!m) return;
      console.log('MapView: planetaryexplorer-select-module event ->', m);
      handleModuleSelect(m);
    };
    window.addEventListener('planetaryexplorer-select-module' as any, handleSelectModuleEvent as any);
    return () => {
      window.removeEventListener('planetaryexplorer-select-module' as any, handleSelectModuleEvent as any);
    };
  ...
  const handleMapClickForPin = async (lat: number, lng: number) => {
    ...
    // terrain, ...) based on the user's question.
    if (freePinMode && !selectedModule) {
      console.log(`[PIN] MapView: Free pin placed at (${lat.toFixed(4)}, ${lng.toFixed(4)})`);

      // Remove existing free-pin marker if present
      if (pinState.marker) {
        try {
          ...
          console.warn('[PIN] MapView: Failed to remove previous free pin marker:', e);
        }
      }
      ...
      if (mapProvider === 'leaflet' && window.L) {
        const pinIcon = window.L.divIcon({
          html: `
            <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#2563eb" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="filter: drop-shadow(0 2px 4px rgba(0,0,0,0.3));">
              <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" fill="#2563eb"></path>
        ...
        marker.bindPopup(`Pin<br/>Lat: ${lat.toFixed(4)}°<br/>Lng: ${lng.toFixed(4)}°`).openPopup();
      } else if (mapProvider === 'azure' && window.atlas) {
        marker = new window.atlas.HtmlMarker({
          position: [lng, lat],
          htmlContent: `<div style="width:36px;height:36px;"><svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#2563eb" stroke-width="2.5"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" fill="#2563eb"/><circle cx="12" cy="10" r="3" fill="white" stroke="none"/></svg></div>`,
          anchor: 'bottom'
        });
      ...
      if (onGeointAnalysis) {
        onGeointAnalysis({
          type: 'info',
          message: `**Pin dropped** at (${lat.toFixed(4)}°, ${lng.toFixed(4)}°)\n\nAsk anything about this location in the chat — for example, *"What is the value here?"* or *"Describe what's at this location."*`
        });
      }
      return;
    }
      ...
      if (onGeointAnalysis) {
        ...
        switch (selectedModule) {
          ...
          case 'extreme_weather':
            ...
            break;
          case 'site_audit':
            apiModule = 'site_audit';
            query = 'Audit this candidate site for AI data center suitability';
            displayName = 'Site Intel';
            break;
          case 'forecast':
            apiModule = 'forecast';
        ...
        if (selectedModule === 'building_damage') {
          ...
          // Exit early - don't trigger automatic analysis
          return;
        } else if (selectedModule === 'site_audit') {
          // SITE AUDIT MODULE: Capture pin only (backend reads Fabric/MPC/AI Search,
          // doesn't need screenshot). Wait for user question in Chat which triggers the audit.
          console.log('MapView: Site Audit module - pin placed, waiting for user question...');

          // Update vision pin state so Chat dispatch can read coords
          setVisionPin({ lat, lng });
          setVisionMode(true);

          onGeointAnalysis({
            type: 'site_audit_ready',
            message: '**Pin Placed**\n\nAsk your question providing relevant details (e.g., target capacity in MW).',
            coordinates: { lat, lng }
          });
          ...
          analysisAbortControllerRef.current = null;
          return;
        } else if (selectedModule === 'forecast') {
          // FORECAST MODULE: Capture pin only. Backend (POST /api/geoint/forecast) runs
          // Aurora + Earth-2 FCN ensemble for this lat/lng; user types the question in Chat.
          console.log('MapView: Forecast module - pin placed, waiting for user question...');
  ...
  // Debounced — Azure Maps fires sourcedata events many times during a single
  // tile load, and serializing the full context on each one was dominating
  // the main thread. 250ms collapses bursts into a single update.
  const mapContextDebounceRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  useEffect(() => {
    ...
    if (mapContextDebounceRef.current) {
      clearTimeout(mapContextDebounceRef.current);
    }
    mapContextDebounceRef.current = setTimeout(() => {
      try {
      ...
      const context = {
        ...
        // Send whenever ANY pin is dropped — the regular Drop Pin button
        // (pinState) or the Vision module's pin (visionPin). Backend reads
        // this as `vision_pin` and uses it for pin+map raster sampling.
        vision_pin: (visionMode && visionPin)
          ? visionPin
          : (pinState.active && pinState.lat != null && pinState.lng != null
              ? { lat: pinState.lat, lng: pinState.lng }
              : null)
      };

      console.log('MapView: ctx updated', {
        sat: context.has_satellite_data,
        coll: context.current_collection,
        tiles: tileUrls.length,
        items: stacItems.length,
      });
  ...
  // and drop coloured HTML markers on the map. Detail payload is the
  // dossier; we render one marker per facility, coloured by severity.
  // ─────────────────────────────────────────────────────────────────────
  useEffect(() => {
    const clearResilienceMarkers = () => {
      if (!map) {
        resilienceMarkersRef.current = [];
        return;
      }
      for (const m of resilienceMarkersRef.current) {
        try {
          if (mapProvider === 'azure' && map.markers) {
            map.markers.remove(m);
          } else if (mapProvider === 'leaflet' && typeof (map as any).removeLayer === 'function') {
    ...
    const onFacilities = (evt: Event) => {
      const detail = (evt as CustomEvent<ResilienceDossier>).detail;
      if (!detail || !map) return;
      clearResilienceMarkers();

      // Compute bounding box for fit-to-facilities
      let minLat = Infinity, maxLat = -Infinity, minLng = Infinity, maxLng = -Infinity;

      for (const fac of detail.facilities) {
        if (!Number.isFinite(fac.lat) || !Number.isFinite(fac.lng)) continue;
        minLat = Math.min(minLat, fac.lat); maxLat = Math.max(maxLat, fac.lat);
        minLng = Math.min(minLng, fac.lng); maxLng = Math.max(maxLng, fac.lng);

        const color = severityColor(fac.severity);
        const score = Math.round(fac.overall_risk ?? 0);
        // Marker sized for legibility at typical Resilience zoom levels:
        // 36px disk with bold 14px score + outer ring so it reads even on
        // satellite imagery. Keep the iconAnchor centered.
        const html = `<div title="${fac.name} — ${fac.severity} (${score})" style="width:36px;height:36px;border-radius:50%;background:${color};border:3px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.45),0 0 0 2px rgba(0,0,0,0.18);display:flex;align-items:center;justify-content:center;color:white;font-size:14px;font-weight:700;font-family:system-ui,-apple-system,Segoe UI,sans-serif;letter-spacing:-0.5px;text-shadow:0 1px 2px rgba(0,0,0,0.4);">${score}</div>`;

        try {
          if (mapProvider === 'azure' && window.atlas) {
            const marker = new window.atlas.HtmlMarker({
              position: [fac.lng, fac.lat],
              htmlContent: html,
              anchor: 'center'
            });
            map.markers.add(marker);
            resilienceMarkersRef.current.push(marker);
          } else if (mapProvider === 'leaflet' && window.L) {
            const icon = window.L.divIcon({
              html,
              className: 'resilience-facility-marker',
              iconSize: [36, 36],
              iconAnchor: [18, 18],
            });
            const marker = window.L.marker([fac.lat, fac.lng], { icon })
              .addTo(map)
              .bindTooltip(`${fac.name} — ${fac.severity} (${score})`);
            resilienceMarkersRef.current.push(marker);
          }
        } catch (e) {
          console.warn('[RESILIENCE] Failed to add marker for', fac.facility_id, e);
        }
      }

      // Fit camera to facilities
      if (Number.isFinite(minLat) && Number.isFinite(maxLat)) {
        const pad = 0.5;
        try {
          if (mapProvider === 'azure' && typeof map.setCamera === 'function') {
            map.setCamera({
          ...
          } else if (mapProvider === 'leaflet' && typeof (map as any).fitBounds === 'function') {
            (map as any).fitBounds([[minLat - pad, minLng - pad], [maxLat + pad, maxLng + pad]]);
          }
        } catch (e) {
          /* swallow */
        }
    ...
    const onClear = () => clearResilienceMarkers();

    window.addEventListener('resilience:facilities', onFacilities as EventListener);
    window.addEventListener('resilience:clear', onClear);
    return () => {
      window.removeEventListener('resilience:facilities', onFacilities as EventListener);
      window.removeEventListener('resilience:clear', onClear);
      clearResilienceMarkers();
    };
  }, [map, mapProvider]);
    ...
    <div className="map" style={{ position: 'relative' }}>
      ...
      {mapLoaded && (
        <>
          ...
          >
            {/* GEOINT Modules icon — 2x2 module grid (distinct from pin
                icons so users don't confuse the picker with a drop-pin). */}
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
          ...
          {/* Universal Drop-Pin button — independent of GEOINT modules.
              Lets the user drop a single pin and ask any question in chat;
              the analysis router decides which analyzer to run. */}
          <div
            onClick={() => {
              const next = !freePinMode;
              setFreePinMode(next);
              // Toggling free pin off should clear any active free pin so the
              // chat stops sending stale coordinates on subsequent /api/query.
              if (!next) {
                if (pinState.marker) {
                  try {
                    if (mapProvider === 'leaflet' && window.L) map.removeLayer(pinState.marker);
                    ...
                    console.warn('[PIN] MapView: Failed to remove free pin marker on toggle off:', e);
                  }
                }
                setPinState({ lat: null, lng: null, active: false, marker: null });
                if (onPinChange) onPinChange(null);
              } else if (onGeointAnalysis) {
                onGeointAnalysis({
                  type: 'info',
                  message: '**Drop Pin mode enabled**\n\nClick anywhere on the map to drop a pin, then ask a question about that location in chat.'
                });
              }
            }}
            title={freePinMode ? 'Drop Pin: ON — click the map to place a pin (click this button again to cancel)' : 'Drop Pin: click the map to place a pin and ask any question about that location'}
            style={{
              position: 'absolute',
              top: '358px', // Directly under the Z-level indicator (300 + 48 + 10 gap) so the left toolbar reads as one tight column.
              left: '10px',
              background: freePinMode ? 'rgba(37, 99, 235, 0.9)' : 'rgba(255, 255, 255, 0.3)',
              color: freePinMode ? 'white' : '#333',
              width: '48px',
              height: '48px',
              borderRadius: '12px',
              display: 'flex',
              ...
              boxShadow: '0 2px 12px rgba(0, 0, 0, 0.15)',
              zIndex: 1000,
              border: freePinMode ? '2px solid rgba(37, 99, 235, 1)' : '1px solid rgba(0, 0, 0, 0.15)',
              cursor: 'pointer',
              userSelect: 'none',
              transition: 'all 0.2s ease',
              backdropFilter: 'blur(10px)'
          ...
          >
            {/* Plus-on-pin icon to differentiate from the modules pin */}
            <svg
              width="22"
              height="22"
              viewBox="0 0 24 24"
          ...
          {/* Data Catalog Toggle - positioned at top of left button stack, under pin button */}
          <div 
            onClick={onToggleSidebar}
            title={sidebarOpen ? "Collapse Data Catalog" : "Expand Data Catalog"}
            style={{
              position: 'absolute',
              top: '68px', // Under pin button (top of left button stack)
              left: '10px',
          ...
          {/* Zoom In Button - positioned under data catalog toggle */}
          <div 
            onClick={handleZoomIn}
            title="Zoom In"
            style={{
              position: 'absolute',
              top: '126px', // Under data catalog toggle
              left: '10px',
              background: 'rgba(255, 255, 255, 0.3)',
              color: '#1a1a1a',
              width: '48px',
          ...
          {/* Zoom Level Indicator - positioned under compass button */}
          <div 
          ...
          {/* Map labels / style toggle - sits below the drop-pin button at
              the bottom of the left toolbar. Lets the user hide the map
              text overlay (place names, road labels, etc.) by swapping
              the Azure Maps basemap between 'satellite_road_labels' and
              'satellite'. On Leaflet fallback the click is a no-op
              (Leaflet basemap doesn't expose label layers). */}
          <div
            onClick={() => {
              const next = !showMapLabels;
              setShowMapLabels(next);
              try {
                if (mapProvider === 'azure' && map && typeof map.setStyle === 'function') {
                  map.setStyle({ style: next ? 'satellite_road_labels' : 'satellite' });
                  console.log(`[STYLE] MapView: Basemap -> ${next ? 'satellite_road_labels' : 'satellite'}`);
                }
              } catch (e) {
                console.warn('[STYLE] MapView: Failed to swap basemap style:', e);
              }
            }}
            title={showMapLabels ? 'Hide map labels (switch to plain satellite)' : 'Show map labels (place names, roads)'}
            style={{
              position: 'absolute',
              top: '416px', // Under the drop-pin button (358 + 48 + 10)
              left: '10px',
              background: showMapLabels ? 'rgba(255, 255, 255, 0.3)' : 'rgba(37, 99, 235, 0.9)',
              color: showMapLabels ? '#1a1a1a' : 'white',
              width: '48px',
              height: '48px',
              borderRadius: '12px',
              display: 'flex',
              ...
              boxShadow: '0 2px 12px rgba(0, 0, 0, 0.15)',
              zIndex: 1000,
              border: showMapLabels ? '1px solid rgba(0, 0, 0, 0.15)' : '2px solid rgba(37, 99, 235, 1)',
              cursor: 'pointer',
              userSelect: 'none',
              transition: 'all 0.2s ease',
              backdropFilter: 'blur(10px)'
          ...
          >
            {/* Stacked-layers icon = "map styles". When labels are
                hidden the button flips to filled-blue with a slash to
                signal the off state. */}
            <svg
              width="22"
              height="22"
              viewBox="0 0 24 24"
      ...
      {/* ResiliencePanel is intentionally NOT mounted: the chat-driven flow
          (Chat.tsx → /api/resilience/assess → dossier in chat + marker event)
          is the supported UX. The map listens for `resilience:facilities`
          to drop the severity-colored circle markers. Keeping the import
          available in case a future iteration brings back a side panel. */}
      {false && (
        <ResiliencePanel
          visible={false}
          onClose={() => {}}
