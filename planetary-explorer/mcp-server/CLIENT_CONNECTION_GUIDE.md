...
- Planetary Explorer MCP server running
...
```json
{
  "mcpServers": {
    "planetary-explorer": {
      "command": "python",
      "args": [
        "C:\\Users\\YourUser\\Planetary-Explorer\\planetary-explorer\\mcp-server\\server.py"
      ],
      "env": {
...
Claude: Yes! I can see the planetary-explorer MCP server with the following capabilities:
...
class PlanetaryExplorerMCPClient:
...
async def main():
    # Initialize client
    client = PlanetaryExplorerMCPClient(
...
class PlanetaryExplorerMCPClientAdvanced:
    def __init__(
        self, 
        base_url: str, 
...
# Usage
async def demo():
    client = PlanetaryExplorerMCPClientAdvanced(
        base_url="https://your-mcp-server.azurewebsites.net",
        api_key="your-key"
    )
...
// Usage Example
async function main() {
  const client = new PlanetaryExplorerMCPClient(
...
import { PlanetaryExplorerMCPClient } from './planetaryExplorerClient';

const SatelliteAnalysis: React.FC = () => {
  const [client] = useState(
    () => new PlanetaryExplorerMCPClient(
...
```python
import asyncio
from planetary_explorer_client import PlanetaryExplorerMCPClient
