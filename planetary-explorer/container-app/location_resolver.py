...
class EnhancedLocationResolver:
    ...
    STORED_LOCATIONS = {
        ...
        'santa rosa': [-122.78, 38.38, -122.65, 38.51],  # Santa Rosa, California (Sonoma County)
        'santa rosa california': [-122.78, 38.38, -122.65, 38.51],  # Alias
        'sonoma county': [-123.53, 38.07, -122.36, 38.85],  # Broader Sonoma County, CA
    ...
    # ------------------------------------------------------------------
    # Type-aware preprocessing constants
    # ------------------------------------------------------------------
    # Natural / landmark feature keywords. If any of these tokens appear
    # in a location name, we MUST NOT append admin-division suffixes such
    # as "state USA" because that misroutes geocoder lookups
    # (e.g. "Lake Tahoe state USA" -> a small lake in Michigan).
    _NATURAL_FEATURE_KEYWORDS = frozenset({
        'lake', 'lakes', 'mount', 'mt', 'mountain', 'mountains', 'peak', 'peaks',
        'river', 'rivers', 'creek', 'brook', 'stream', 'falls', 'waterfall',
        'canyon', 'canyons', 'gorge', 'valley', 'valleys', 'basin', 'plateau',
        'range', 'ridge', 'butte', 'mesa', 'dune', 'dunes',
        'bay', 'gulf', 'sound', 'strait', 'channel', 'cove', 'fjord', 'harbor',
        'harbour', 'reef', 'archipelago',
        'cape', 'peninsula', 'delta', 'estuary',
        'desert', 'forest', 'rainforest', 'jungle', 'savanna', 'tundra',
        'preserve', 'wilderness', 'reserve', 'sanctuary',
        'glacier', 'icefield', 'volcano', 'crater',
        'geyser',
    })

    # Multi-word natural-feature phrases (checked via substring containment).
    _MULTI_WORD_NATURAL_FEATURES = (
        'national park', 'state park', 'national forest', 'national monument',
        'national seashore', 'national recreation area', 'wildlife refuge',
        'hot springs',
    )

    # Anchor hints for famous US natural features whose names confuse
    # general-purpose geocoders. Used as the FIRST preprocessed query.
    _KNOWN_LANDMARK_HINTS = {
        'lake tahoe': 'Lake Tahoe California Nevada',
        'lake powell': 'Lake Powell Utah Arizona',
        'lake mead': 'Lake Mead Nevada Arizona',
        ...
        'mount whitney': 'Mount Whitney California',
        'mount shasta': 'Mount Shasta California',
        'mount rainier': 'Mount Rainier Washington',
        'mount hood': 'Mount Hood Oregon',
        'mount st helens': 'Mount Saint Helens Washington',
        'mount saint helens': 'Mount Saint Helens Washington',
        'mount mckinley': 'Denali Alaska',
        'denali': 'Denali Alaska',
        'half dome': 'Half Dome Yosemite California',
        'el capitan': 'El Capitan Yosemite California',
    }

    # Continental US sanity-check bbox (lon_w, lat_s, lon_e, lat_n).
    ...
    async def resolve_location_to_bbox(self, location_name: str, location_type: str = "region") -> Optional[List[float]]:
        ...
        # Also try a comma-stripped variant: many hardcoded keys are stored
        # without commas ("washington dc", "new york ny") but users naturally
        # type them with commas ("Washington, DC", "New York, NY"). Without
        # this, the comma-split fallback below would strip the qualifier and
        # match the wrong entity -- e.g. "Washington, DC" splitting to
        # "washington" hits the Washington STATE bbox instead of the city.
        loc_comma_collapsed = re.sub(r"\s*,\s*", " ", location_lower).strip()
        loc_norm_comma_collapsed = re.sub(r"\s*,\s*", " ", location_normalized).strip()
        for loc_variant in [
            location_lower,
            location_normalized,
            loc_comma_collapsed,
    ...
    async def _strategy_azure_maps(self, location_name: str, location_type: str = "region") -> Optional[List[float]]:
        """[BLUE] Azure Maps Search with type-aware routing, confidence and country sanity checks.

        Routing order:
          0. Natural features / landmarks  -> POI+Land entity search (skips admin path)
          1. Admin divisions (states/provinces/countries) -> structured geocoding
          2. General fuzzy search with intelligent ranking
          3. Population-priority search for ambiguous city names
          4. Address search fallback
        """
        # Trust the name over caller-supplied type when they conflict
        location_type = self._normalize_location_type(location_name, location_type)
        is_natural = self._looks_like_natural_feature(location_name)
        is_admin = self._looks_like_admin_division(location_name)

        # Strategy 0: Natural features / landmarks must run BEFORE the admin
        # path. Otherwise "Lake Tahoe" gets routed to structured admin
        # search and the geocoder picks an unrelated Michigan lake.
        if is_natural or location_type in ('natural_feature', 'landmark', 'body_of_water', 'mountain', 'park'):
            bbox = await self._azure_maps_landmark_search(location_name)
            if bbox:
                self.logger.info(f"[BLUE] Landmark/natural-feature bbox for '{location_name}': {bbox}")
                return bbox

        # Strategy 1: Structured admin geocoding (states/provinces/countries).
        # Gated on (a) caller said admin OR name looks admin AND (b) name is
        # not actually a natural feature.
        if (location_type in ('state', 'province', 'country') or is_admin) and not is_natural:
    ...
    async def _azure_maps_landmark_search(self, location_name: str) -> Optional[List[float]]:
        """Azure Maps search tuned for landmarks and natural features.

        Uses ``entityType=POI,Land`` (points of interest + lakes/mountains/parks)
        and rejects results below a confidence threshold or outside the
        expected country envelope. This is the dedicated path that prevents
        the historical "Lake Tahoe -> Michigan" misroute.
        """
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(url, params=params, headers=headers) as resp:
                    if resp.status != 200:
                        self.logger.debug(f"Landmark search HTTP {resp.status} for '{location_name}'")
                        return None
            ...
            if not results:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(url, params=params2, headers=headers2) as resp2:
                        if resp2.status == 200:
                            data2 = await resp2.json()
                            results = data2.get("results", [])
            if not results:
                return None
            ranked = self._rank_results_by_relevance(results, location_name)
            for r in ranked:
                if not self._is_high_confidence_result(r, min_score=3.0):
                    continue
                bbox = self._extract_azure_bounds(r)
                if not bbox:
                    continue
                poi_name = (r.get('poi') or {}).get('name') or (r.get('address') or {}).get('freeformAddress', 'N/A')
                self.logger.info(
                    f"[BLUE] Landmark/POI match: '{poi_name}' score={r.get('score', 0):.2f} "
                    f"country={self._result_country_code(r)} -> {bbox}"
                )
                return bbox
        except Exception as e:
            self.logger.error(f"Azure Maps landmark search error: {e}")
        return None
    ...
    def _looks_like_city(self, location_name: str) -> bool:
        ...
        if self._looks_like_admin_division(location_name):
            return False
        # If it's a natural feature (lake/mount/river/etc.), it's not a city
    ...
    # ------------------------------------------------------------------
    # Type-aware classification helpers (long-term fix for Tahoe -> Michigan
    # class of bug: caller-supplied location_type is unreliable, so we
    # re-classify based on the name itself).
    # ------------------------------------------------------------------
    def _looks_like_natural_feature(self, location_name: str) -> bool:
        """True iff the name contains a natural-feature keyword.

        Detects lakes, mountains, rivers, canyons, bays, parks, forests,
        glaciers, etc. These must bypass admin-division preprocessing
        because suffixes like "state USA" geocode to wrong locations.
        """
        ...
        for art in ('the ', 'a ', 'an '):
            if name_lower.startswith(art):
                name_lower = name_lower[len(art):].strip()
        # Multi-word phrases first (substring containment)
        for phrase in self._MULTI_WORD_NATURAL_FEATURES:
            if phrase in name_lower:
                return True
        # Single-word tokens
        tokens = set(name_lower.replace('-', ' ').replace(',', ' ').split())
        return bool(tokens & self._NATURAL_FEATURE_KEYWORDS)

    def _normalize_location_type(self, location_name: str, declared_type: str) -> str:
        """Re-classify a location_type when the caller's hint disagrees with the name.

        Many callers pass ``location_type='region'`` as a default even when
        the extracted entity is actually a landmark, city, or natural
        feature. We trust the *name* over the *type hint* in those cases.

        Returns one of: 'natural_feature', 'landmark', 'city', 'state',
        ...
        if declared and declared not in ('region', 'unknown', ''):
            return declared
        # Name shape suggests a city
        if self._looks_like_city(location_name):
            return 'city'
        return declared or 'region'

    def _is_high_confidence_result(self, result: Dict, min_score: float = 4.0) -> bool:
        """Reject low-confidence Azure Maps results.

        Azure Maps' ``score`` field is roughly 0-10 (higher = better
        match). Results below 4 are typically far-fetched fuzzy matches
        such as the geocoder picking a random Michigan lake when asked
        for "Lake Tahoe state USA".
        """
        try:
    ...
    def _result_country_code(self, result: Dict) -> Optional[str]:
        """Extract ISO country code from an Azure Maps result, upper-cased."""
        addr = result.get('address') or {}
        code = addr.get('countryCode') or addr.get('countryCodeISO3')
        return code.upper() if code else None

    def _is_in_conus_bbox(self, bbox: List[float]) -> bool:
        """Centroid sanity check: is this bbox inside the continental US envelope?"""
        if not bbox or len(bbox) != 4:
            return False
        cx = (bbox[0] + bbox[2]) / 2.0
        cy = (bbox[1] + bbox[3]) / 2.0
        cw, cs, ce, cn = self._CONUS_BBOX
        return cw <= cx <= ce and cs <= cy <= cn
...
Location: {location_name}"""
    ...
    async def _nominatim_single_query(self, query: str) -> Optional[List[float]]:
        ...
        headers = {"User-Agent": "PlanetaryExplorer/2.0 (geographic-analysis)"}
    ...
    def _preprocess_location_query(self, location_name: str, location_type: str) -> List[str]:
        ...
        - "Lake Tahoe" -> ["Lake Tahoe California Nevada", "Lake Tahoe"]  (NO "state USA" suffix)
        """
        # Re-classify when the caller's type hint conflicts with the name.
        # e.g. semantic_translator hardcodes 'region' but "Lake Tahoe" is a
        # natural feature; without this, the resolver would append "state
        # USA" and the geocoder would return a Michigan lake.
        location_type = self._normalize_location_type(location_name, location_type)
        processed_queries = []
        name_lower = location_name.lower().strip()
        ...
        elif self._looks_like_natural_feature(location_name) or location_type in ('natural_feature', 'landmark', 'body_of_water', 'mountain', 'park'):
            # Natural features (lakes, mountains, parks, rivers) — NEVER
            # append "state USA". Use anchor hints for well-known landmarks.
            hint = self._KNOWN_LANDMARK_HINTS.get(name_lower)
            if hint:
                processed_queries.append(hint)
            processed_queries.extend([
                f"{location_name}",
                f"{location_name} United States",
            ])
    ...
    async def _nominatim_international_query(self, query: str) -> Optional[List[float]]:
        ...
        headers = {"User-Agent": "PlanetaryExplorer/2.1 (enhanced-international-geocoding)"}
    ...
    def _rank_results_by_relevance(self, results: List[Dict], location_name: str) -> List[Dict]:
        """Rank Azure Maps results by relevance.

        Combines Azure Maps' own ``score`` (the strongest signal) with
        heuristics for name match and entity type. The Azure Maps score
        is weighted heavily (x5) so a clearly-better fuzzy match always
        outranks a weak fuzzy match with the right entity type.
        """
        if not results:
            return results
        name_lower = (location_name or '').lower().strip()
        is_natural = self._looks_like_natural_feature(location_name)
        
        def calculate_score(result):
            score = 0.0
            address = result.get('address') or {}
            entity_type = result.get('entityType') or ''
            poi = result.get('poi') or {}
            
            # 1) Azure Maps' own fuzzy-match score (~0-10). Strongest signal.
            try:
                score += float(result.get('score', 0) or 0) * 5.0
            except (TypeError, ValueError):
                pass

            # 2) Exact name match against POI name or municipality
            poi_name = (poi.get('name') or '').lower()
            municipality = (address.get('municipality') or '').lower()
            freeform = (address.get('freeformAddress') or '').lower()
            if name_lower and (poi_name == name_lower or municipality == name_lower):
                score += 50
            elif name_lower and name_lower in freeform:
                score += 20
            
            # 3) Entity-type preferences
            if is_natural:
                # Natural features should prefer POI/Land entity types over
                # municipalities (otherwise "Lake Tahoe" matches a town).
                if 'POI' in entity_type or 'Land' in entity_type:
                    score += 30
            ...
            # 4) Prefer results with state context
