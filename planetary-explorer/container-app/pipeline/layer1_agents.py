...
class LoadAndAnalyzeAgent(Executor):  # type: ignore[misc]
    ...
    async def run(
        ...
        #
        # Previously we propagated AnalyzeAgent's CLARIFY verbatim, which
        # caused the FastAPI handler to short-circuit before rendering
        # the tiles the LOAD half already resolved. For Get Started
        # prompts ("show fire MODIS thermal anomalies daily activity for
        # Australia from June 2025"), the LoadAgent picks the right
        # collection + bbox + time window cleanly, but the AnalystAgent
        # has no pin / no screenshot and falls back to ask_user_to_clarify
        # — blocking the whole turn on a question the user never asked.
        #
        # Fix: when LOAD has a real plan (action=LOAD with a stac_query),
        # demote ANALYZE's clarify into a soft follow-up appended to the
        # chat. Return the LOAD action so tiles render normally; the user
        # can answer the follow-up on the next turn to deepen analysis.
        # ------------------------------------------------------------------
        if (
            analyze_result.get("action") == "CLARIFY"
            and load_result.get("action") == "LOAD"
            and (load_result.get("stac_query") or "")
        ):
            follow_up = (analyze_result.get("answer") or "").strip()
            load_summary = (load_result.get("answer") or "").strip()
            combined = (
                f"{load_summary}\n\n{follow_up}"
                if load_summary and follow_up
                else (follow_up or load_summary)
            )
            load_result["answer"] = combined
            # Surface the analyst's chip options as soft suggestions,
            # not a blocking clarification round.
            _analyze_struct = analyze_result.get("structured") or {}
            _soft_opts = list(_analyze_struct.get("options") or [])
            if _soft_opts:
                merged_struct = dict(load_result.get("structured") or {})
                merged_struct["soft_follow_up_options"] = _soft_opts
                load_result["structured"] = merged_struct
            load_result["elapsed_ms"] = int((time.time() - started) * 1000)
            logger.info(
                "[LOAD_AND_ANALYZE] LOAD succeeded; ANALYZE wanted clarify "
                "-> rendering LOAD with appended follow-up (q=%r, opts=%d)",
                follow_up[:80],
                len(_soft_opts),
            )
            return load_result
