...
- Prioritize quality over quantity"""
    ...
    def _resolve_temporal_to_datetime(self, entities: Optional[Dict[str, Any]], collections: List[str]) -> Optional[str]:
        ...
        # Static collections (DEM, land-cover annuals) and composites (MODIS) don't use datetime
        static_collections = [
            "cop-dem-glo-30", "cop-dem-glo-90", "3dep-seamless", "alos-dem",
            # Land-cover / land-use products are static-vintage rasters; the
            # datetime-parsing agent picks "recent" windows that don't intersect
            # the dataset vintages and the search returns 0 features.
            "io-lulc-9-class", "io-lulc-annual-v02", "esa-worldcover", "usda-cdl",
        ]
...
NAMED EVENTS (use these dates when a query references an event by name):
- "Hurricane Harvey" / "Harvey" -> "2017-08-25/2017-09-03" (Houston, TX)
- "Hurricane Katrina" / "Katrina" -> "2005-08-23/2005-08-31" (New Orleans, LA)
- "Hurricane Sandy" / "Sandy" / "Superstorm Sandy" -> "2012-10-22/2012-11-02" (NJ/NY)
- "Hurricane Maria" / "Maria" -> "2017-09-16/2017-09-30" (Puerto Rico)
- "Hurricane Ian" / "Ian" -> "2022-09-23/2022-10-02" (FL)
- "Hurricane Irma" / "Irma" -> "2017-09-06/2017-09-13" (FL/Caribbean)
- "Hurricane Dorian" / "Dorian" -> "2019-08-24/2019-09-07" (Bahamas/SE US)
- "Hurricane Michael" / "Michael" -> "2018-10-07/2018-10-16" (FL panhandle)
- "Hurricane Ida" / "Ida" -> "2021-08-26/2021-09-03" (LA/NE US)
- "Hurricane Helene" / "Helene" -> "2024-09-24/2024-09-29" (FL/NC)
- "Hurricane Milton" / "Milton" -> "2024-10-05/2024-10-11" (FL)
- "Camp Fire" -> "2018-11-08/2018-11-25" (Paradise, CA)
- "Dixie Fire" -> "2021-07-13/2021-10-25" (Butte County, CA)
- "Marshall Fire" -> "2021-12-30/2022-01-02" (Boulder, CO)
- "Thomas Fire" -> "2017-12-04/2018-01-12" (Ventura, CA)
- "Maui Fire" / "Lahaina Fire" -> "2023-08-08/2023-08-16" (Maui, HI)
- "Tohoku earthquake" / "Fukushima" -> "2011-03-11/2011-03-20" (Japan)
- "Turkey earthquake" / "Kahramanmaras" -> "2023-02-06/2023-02-20" (Turkey/Syria)
- "Tonga eruption" / "Hunga Tonga" -> "2022-01-13/2022-01-20" (Tonga)
If a query mentions one of these names, USE THE CORRESPONDING DATE RANGE even if no explicit date is given.
    ...
    async def _resolve_via_nominatim(self, location_name: str) -> Optional[List[float]]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                headers = {"User-Agent": "PlanetaryExplorer/1.0"}
...
boundary for cities and the full administrative boundary for regions or countries."""
    ...
    async def translate_query(self, natural_query: str, pin_location: Optional[Dict[str, float]] = None, session_bbox: Optional[List[float]] = None, skip_intent_classification: bool = False, pre_classified_intent: Optional[str] = None) -> Dict[str, Any]:
        ...
        # ====================================================================
        # [PIN-OVERRIDE PRE-CHECK] Demo-flow safety net
        # --------------------------------------------------------------------
        # If the user's query is clearly a NEW navigation/load (e.g.
        # "Show elevation map of Mount Rainier, Washington") AND a pin is
        # still active from a prior turn, the pin would otherwise clamp the
        # bbox to the OLD location and the STAC search returns 0 results.
        #
        # We pre-clear the pin and session_bbox here so Agent 2 is free to
        # geocode the explicit place name in the query. The downstream
        # priority logic still works for follow-ups that don't mention a new
        # place name (those keep the pin).
        # ====================================================================
        if pin_location or session_bbox:
            import re as _re
            _q = natural_query.strip()
            _has_nav_verb = bool(_re.match(
                r"^(?:show|find|display|map|imagery of|elevation map of|elevation of|"
                r"navigate to|go to|take me to|fly to|zoom to|locate|search for)\b",
                _q,
                _re.IGNORECASE,
            ))
            # Heuristic for "explicit place": a comma-separated location
            ...
            if _has_nav_verb and _has_explicit_place:
                logger.info(
                    ...
                    "clearing stale pin/session_bbox so Agent 2 can geocode."
                )
                pin_location = None
                session_bbox = None
    ...
    async def generate_empty_result_response(
        ...
        """
        # ----------------------------------------------------------------
        # Fast-fail short-circuit (default ON): when STAC returns 0 hits,
        # do NOT spin up another LLM call to write the apology prose. Use
        # the deterministic fallback string. Saves 5-15 s per dead-end
        # query and is the right UX (the user wants to retry, not read).
        # Set EMPTY_RESULT_FAST_FAIL=0 to restore the LLM-prose path.
        # ----------------------------------------------------------------
        import os as _os
        if _os.environ.get("EMPTY_RESULT_FAST_FAIL", "1") != "0":
    ...
    async def _fallback_contextual_response(self, query: str, classification: Dict[str, Any], stac_response: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                _ctx_result = await get_contextual_agent().run(
                    ContextualInput(question=query, hint=None, history=[])
                )
                if _ctx_result.success and _ctx_result.answer:
                    return {
                        "message": _ctx_result.answer,
                        "query_type": "v2_contextual_agent",
                        "has_satellite_data": stac_response is not None and stac_response.get("success", False),
                        "has_contextual_analysis": True,
                        "location_focus": classification.get("location_focus"),
                        "fallback_used": True,
                        "method": "v2_contextual_agent",
                    }
            except Exception as _v2_err:
                logger.error(f"V2 ContextualAgent fallback also failed: {_v2_err}")
            return {
                "message": (
                    "I couldn't generate a contextual answer for that query "
                    "(LLM call failed). Please rephrase or try again."
                ),
                "query_type": "error",
...
Keep your response focused, informative, and directly relevant to the user's question."""
    ...
    def _prepare_stac_data_summary(self, features: List[Dict], collection_summary: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Fallback for STAC items where `datetime` is null but a date range is
        # expressed via `start_datetime` / `end_datetime` (ESA WorldCover,
        # Copernicus DEM, some Landsat/HLS composites, etc.).
        if not dates:
            range_starts = [
                f.get("properties", {}).get("start_datetime")
                for f in features
                if f.get("properties", {}) and f.get("properties", {}).get("start_datetime")
            ]
            range_ends = [
                f.get("properties", {}).get("end_datetime")
                for f in features
                if f.get("properties", {}) and f.get("properties", {}).get("end_datetime")
            ]
            if range_starts and range_ends:
                earliest = min(range_starts)[:10]
                latest = max(range_ends)[:10]
                if earliest == latest:
        ...
        if dates:
            ...
            if earliest == latest:
                ...
                temporal_coverage = f"Date range: {earliest} to {latest}"
        elif temporal_coverage == "Unknown":
