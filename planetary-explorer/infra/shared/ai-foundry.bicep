...
// Deploy GPT-4.1-mini model (faster, cheaper option — successor to gpt-4o-mini)
...
resource hub 'Microsoft.MachineLearningServices/workspaces@2024-10-01' = if (deployAgentService && !empty(hubName)) {
  ...
  properties: {
    friendlyName: 'Planetary Explorer AI Hub'
  ...
  // Wait for ALL CognitiveServices/accounts/deployments to finish before
  // creating the hub or aiServicesConnection — both touch the parent account.
  dependsOn: [
    aiFoundry
    embeddingDeployment
  ]
...
// IMPORTANT: depends on embeddingDeployment to fully serialize against the
// model-deployment chain (all model PUTs write to children of aiFoundry and
// can collide with this connection PUT, producing RequestConflict).
...
resource project 'Microsoft.MachineLearningServices/workspaces@2024-10-01' = if (deployAgentService && !empty(projectName)) {
  ...
  properties: {
    friendlyName: 'Planetary Explorer Agent Project'
...
// IMPORTANT: depends on BOTH the ML workspace project AND embeddingDeployment.
// embeddingDeployment is the tail of the model-deployment chain (which all write
// to children of aiFoundry). Without that dependency, this PUT can race with a
// model deployment PUT during idempotent redeploys, producing RequestConflict
// on the parent CognitiveServices account.
resource agentProject 'Microsoft.CognitiveServices/accounts/projects@2025-04-01-preview' = if (deployAgentService) {
  ...
  dependsOn: [
    ...
    embeddingDeployment // Serialize against model-deployment chain to prevent RequestConflict
  ]
}
